...
onMounted(async () => {
  try {
    await vaultStore.read();
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    toast.add({ title: 'Vaults konnten nicht geladen werden', description: message, color: 'error' });
  }
});
...
<template>
  <UDashboardSidebar>
    ...
    <template #default>
      <UNavigationMenu :items="vaultItems" orientation="vertical" />
